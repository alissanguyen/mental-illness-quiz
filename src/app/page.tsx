...
export default function Home() {
  ...
  React.useEffect(() => {
    ...
    if (savedData) {
      const { userName, answers } = JSON.parse(savedData);
      console.log('Saved data 2:', answers);
  ...
  const handleContinue = (answer: boolean) => {
    ...
    if (answer) {
      loadQuizData(); // Load the quiz data, which internally checks for validity
    } else {
      clearQuizData(); // Clears all data to start fresh
    }
    setIsDoingQuiz(true); // Ensure this is always set correctly based on the condition
  };
